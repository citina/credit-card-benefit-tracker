...
- **`Catalog`** (11 cols): `Card, LastVerified, Benefit, Amount, Category, Reset, ReminderDays, SourceUrl, PeriodBasis, Notes, Issuer`. `getCatalogData_` reads it **by header name** (not column position); `ensureHeaders_` append-migrates older 7-col sheets. `LastVerified` is plain-text (`@`) on fresh installs; `verifiedKey_` normalizes any coerced-Date cell on read.
- **Catalog audit refresh:** `setup()` preserves existing Catalog rows by design. After accepting a shipped audit that changes existing row values, run editor-only `syncCatalogFromConstant()` once to refresh matching shipped card+benefit rows and append new ones. If annual fees changed for cards already added to the tracker, also run `syncCardFeesFromConstant()` because the dashboard reads annual fees from the `Cards` sheet. Then deploy a New version and hard-refresh the web page.
...
2b. **Conditional / spend-gated benefits.** Some perks unlock only after annual spend, and the tracker has **no spend/transaction data** so it can't auto-unlock them. Workable today: add such a credit only once unlocked, with the caveat in `Notes` ("after $75k spend"). Non-$ **value-on-use** perks that expire (free-night awards, companion certificates) are in scope; built-in status/lounge access, points boosts, and earning multipliers stay out of scope. Possible later: a `Notes`-surfacing or an "unlocked?" toggle.
